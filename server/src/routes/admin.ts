...
router.get(
  ...
  (req, res) => {
    ...
    const countResult = db.prepare(countQuery).get(...countParams) as { total: number } || { total: 0 };
...
router.get(
  ...
  (req, res) => {
    try {
      ...
      // Total users
      const totalUsersResult = db.prepare('SELECT COUNT(*) as count FROM users').get() as { count: number } || { count: 0 };
      ...
      // Week-over-week changes (current vs previous week)
      ...
      const previousWeek: WeekMetrics = {
