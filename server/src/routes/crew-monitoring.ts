...
const emotionDetectionSchema = z.object({
  ...
  location: z.string().optional(), // e.g., "command_module", "sleep_quarters"
});

// Critical issue detection schema
const criticalIssueSchema = z.object({
  crewMemberId: z.string(),
  issueType: z.enum(['emotional_distress', 'physical_discomfort', 'sleep_disruption', 'crew_conflict', 'mission_stress']),
...
router.post("/analyze-media", requireAuth, upload.single('media'), async (req, res) => {
  try {
    if (!req.file) {
    ...
    `).run(
      ...
      sessionId || null,
      req.file.path,
      JSON.stringify(analysisResult.emotions),
...
router.get("/crew/:crewMemberId/emotions", requireAuth, async (req, res) => {
  try {
...
router.get("/critical-issues", requireAuth, async (req, res) => {
  try {
    ...
    res.json({
      issues,
        summary: {
          ...
          byCrewMember: issues.reduce((acc: any, issue: any) => {
...
router.post("/ai-companion", requireAuth, async (req, res) => {
  try {
...
// Helper functions
async function processVideoEmotion(filePath: string) {
...
async function processAudioEmotion(filePath: string) {
...
async function processImageEmotion(filePath: string) {
...
async function detectCriticalIssues(analysisResult: any, crewMemberId: string) {
  const issues = [];
...
function generateRecommendations(analysisResult: any) {
...
async function generateAICompanionResponse(message: string, context: any) {
...
function generateEmotionalSummary(emotions: any[]) {
  ...
  const dominantEmotion = Object.keys(avgConfidence).reduce((a, b) => 
    avgConfidence[a] > avgConfidence[b] ? a : b
  );
