...
const activitiesSeed_1 = require("../data/activitiesSeed");
const router = (0, express_1.Router)();
// Utility to seed if empty
const ensureSeeded = () => {
    const db = (0, setup_1.getDb)();
    const count = db.prepare("SELECT COUNT(1) as c FROM activities").get();
    if (count.c === 0) {
    ...
    `);
        ...
        insertMany(activitiesSeed_1.activitiesSeed);
    }
    else {
        // Backfill video URLs for known activities if they were seeded before video_url existed
        db.prepare(`UPDATE activities SET video_url = ? WHERE id = 'breath-work' AND (video_url IS NULL OR video_url = '')`).run('https://youtu.be/I77hh5I69gA?si=wrV-MNiglXpPP_Bw');
        db.prepare(`UPDATE activities SET video_url = ? WHERE id = 'body-scan' AND (video_url IS NULL OR video_url = '')`).run('https://youtu.be/5mOZMxVKmiY?si=67NMCQ6llE_xBQ6U');
    }
    db.close();
};
...
router.post("/", (req, res) => {
  ...
  `);
    stmt.run(body.id, body.title, body.description || "", body.category || "mindfulness", body.duration || "10-15 min", body.difficulty || "Easy", JSON.stringify(body.benefits || []), body.icon || null, body.color || null, body.recommendedFor ? JSON.stringify(body.recommendedFor) : null, body.videoUrl || null);
    db.close();
...
// Mark completion
router.post("/:id/complete", middleware_1.requireAuth, (req, res) => {
    const userId = req.user.userId;
    const notes = (req.body && req.body.notes) || null;
    const db = (0, setup_1.getDb)();
    db.prepare("INSERT INTO activity_completions (user_id, activity_id, notes) VALUES (?, ?, ?)").run(userId, req.params.id, notes);
    db.close();
    res.json({ ok: true });
});
// Unmark completion (toggle off)
router.delete("/:id/complete", middleware_1.requireAuth, (req, res) => {
    const userId = req.user.userId;
    const db = (0, setup_1.getDb)();
    const info = db
        .prepare(`DELETE FROM activity_completions WHERE user_id = ? AND activity_id = ?`)
        .run(userId, req.params.id);
    db.close();
    res.json({ ok: true, changes: info.changes });
});
// Get completions for current user
router.get("/me/completions", middleware_1.requireAuth, (req, res) => {
    const userId = req.user.userId;
    const db = (0, setup_1.getDb)();
    const rows = db
        .prepare(`SELECT ac.*, a.title, a.category, a.duration, a.difficulty
...
// Stats
router.get("/me/stats", middleware_1.requireAuth, (req, res) => {
    const userId = req.user.userId;
    const db = (0, setup_1.getDb)();
    const totals = db.prepare(`SELECT COUNT(*) as completed FROM activity_completions WHERE user_id = ?`).get(userId);
    const byCategory = db
        .prepare(`SELECT a.category, COUNT(*) as count
...
router.get("/recommendations", (req, res) => {
    ...
    if (mood) {
        ...
        if (m === 'stressed') {
            recommended = recommended.filter((a) => a.category === 'mindfulness' || a.category === 'relaxation' || a.benefits.some((b) => b.toLowerCase().includes('stress')));
        }
        else if (m === 'anxious') {
            recommended = recommended.filter((a) => a.category === 'mindfulness' || a.benefits.some((b) => b.toLowerCase().includes('anxiety')));
        }
        else if (m === 'low-energy') {
            recommended = recommended.filter((a) => a.category === 'physical' || a.benefits.some((b) => b.toLowerCase().includes('energy')));
        }
        else if (m === 'creative') {
