...
router.get("/availability", (req, res) => {
    ...
    if (!counselor_id || !date)
        return res.status(400).json({ error: "counselor_id and date are required" });
    const db = (0, setup_1.getDb)();
    const rows = db
        .prepare(`SELECT time_slot, is_booked FROM counselor_availability WHERE counselor_id = ? AND date = ? ORDER BY time_slot`)
...
router.post("/availability", (req, res) => {
    ...
    const db = (0, setup_1.getDb)();
    const upsert = db.prepare(`INSERT INTO counselor_availability (counselor_id, date, time_slot, is_booked) VALUES (?, ?, ?, 0)
...
router.post("/book", (req, res) => {
    ...
    const slot = db
        .prepare(`SELECT id, is_booked FROM counselor_availability WHERE counselor_id = ? AND date = ? AND time_slot = ?`)
        .get(counselor_id, date, time_slot);
    if (!slot) {
        db.close();
    ...
    const tx = db.transaction(() => {
        db.prepare(`INSERT INTO bookings (counselor_id, date, time_slot, name, email, phone, issue, urgency, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(counselor_id, date, time_slot, name, email, phone ?? null, issue ?? null, urgency ?? null, notes ?? null);
    ...
    const created = db
        .prepare(`SELECT b.*, c.name as counselor_name, c.specialization FROM bookings b JOIN counselors c ON b.counselor_id = c.id
...
router.get("/list", (req, res) => {
    ...
    const db = (0, setup_1.getDb)();
    const rows = db
        .prepare(`SELECT b.*, c.name as counselor_name, c.specialization
